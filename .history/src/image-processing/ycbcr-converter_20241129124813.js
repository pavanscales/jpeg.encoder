...
export function convertImageToYCbCr(data) {
    ...
    // Assuming the data is in RGBA format and we have the width/height info
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // Convert RGB to YCbCr (simple formula)
      let y = 0.299 * r + 0.587 * g + 0.114 * b;
      let cb = -0.169 * r - 0.331 * g + 0.499 * b + 128;
      let cr = 0.499 * r - 0.419 * g - 0.0813 * b + 128;
  
      // Ensure proper formatting of the result
      if (i / 4 % width === 0) yCbCrImage.push([]);  // Add new row
  
      yCbCrImage[Math.floor(i / (4 * width))].push([y, cb, cr]);
    }
  
    console.log('YCbCr Image:', yCbCrImage);  // Log YCbCr image for inspection
    return yCbCrImage;
  }
