...
export function convertImageToYCbCr(data) {
    ...
    const width = 256;  // Example width (update as per your image's size)
    const height = data.length / width;
    
    const yCbCrImage = [];
    
    // Process each pixel (RGBA) and convert it to YCbCr
    for (let i = 0; i < data.length; i += 4) {
      let r = data[i];
      let g = data[i + 1];
      let b = data[i + 2];
  
      // Conversion from RGB to YCbCr (simplified)
      let y = 0.299 * r + 0.587 * g + 0.114 * b;
      let cb = -0.169 * r - 0.331 * g + 0.499 * b + 128;
      let cr = 0.499 * r - 0.419 * g - 0.0813 * b + 128;
  
      // Store Y, Cb, Cr values in a 2D array format (example)
      if (i / 4 % width === 0) yCbCrImage.push([]);  // Start new row when reaching end of width
      yCbCrImage[Math.floor(i / (4 * width))].push([y, cb, cr]);
    }
    
    console.log('Converted YCbCr Image:', yCbCrImage);  // Log to check the structure
