...
import { encodeHuffmanAC,encodeHuffmanDC } from "./huffaman.js";
function encodeEntropy(data){
    const bitstream = new BitStream()
data.forEach(value=>{
    if(value==='DC'){
        bitstream.writeBits(encodeHuffmanDC())
    }
else{
    bitstream.writeBits(encodeHuffmanAC(1))
}
