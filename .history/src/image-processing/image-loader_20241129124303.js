...
export function loadImageData(imagePath, callback) {
  sharp(imagePath)
    ...
    .then((data) => {
      ...
      // Process the image to extract pixel data (assuming RGBA)
      const width = 256;  // Example width, change if necessary
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        const g = data[i + 1];
        const b = data[i + 2];
        const a = data[i + 3];  // Alpha channel (may or may not be used)

        imageData.push([r, g, b, a]);
      }

      callback(imageData);  // Pass the loaded image data to the callback
    })
    .catch((err) => {
